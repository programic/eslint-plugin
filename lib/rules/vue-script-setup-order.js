...
const getStateNodesThatAreAbsentInOtherGroups = () => {
  ...
  const nonStateVariableNodes = nonStateNodes.map(getParentNode).filter(isVariableNode);

  return states.filter(stateNode => !nonStateVariableNodes.includes(stateNode)).map(getParentNode);
};
